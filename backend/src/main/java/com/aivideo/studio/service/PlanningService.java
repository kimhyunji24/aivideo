...
public class PlanningService {
    ...
    public ProjectState generatePlot(String sessionId, int stageCount, String userPrompt) {
        ...
        String prompt = String.format(
            ...
            "각 단계는 반드시 연속된 2개의 씬으로 구성해야 해 (예: 3단계면 총 6개, 4단계면 총 8개, 5단계면 총 10개).\n" +
            "같은 단계의 두 씬(1/2, 2/2)은 하나의 연속된 장면처럼 이어져야 해:\n" +
            "- 동일한 장소·시간대·등장인물을 유지해.\n" +
            "- 1/2 씬에서 시작된 행동이나 감정이 2/2 씬에서 직접적으로 이어지거나 심화되어야 해.\n" +
            "- 두 씬 사이에는 컷 전환만 있을 뿐, 시간이나 공간이 갑자기 바뀌지 않아야 해.\n" +
            "각 객체는 반드시 'content'와 함께 'elements' 구조를 가져야 해.\n" +
            "추가 사용자의 요구사항이 있다면 반드시 반영해.\n\n" +
            "로그라인: %s\n" +
            "캐릭터 정보: %s\n" +
            "사용자 요구사항: %s\n\n" +
            "반환할 JSON 포맷:\n" +
            ...
            "      \"id\": \"stage-0\",\n" +
            "      \"label\": \"발단 (1/2)\",\n" +
            "      \"content\": \"300~500자 분량의 상세한 단일 장면 묘사. 인물의 표정·의상·동작을 구체적으로 서술하고, 배경의 세부 환경(빛의 방향, 계절감, 공간감)과 카메라 움직임(트래킹 샷, 줌인 등), 사운드·분위기까지 영화 촬영 지문처럼 풍부하게 작성해.\",\n" +
            "      \"elements\": {\n" +
            "        \"mainCharacter\": \"메인 인물 (외형·인상·의상 포함)\",\n" +
            ...
            "        \"story\": \"해당 씬의 핵심 서사와 시각적 묘사 요약\"\n" +
            "      }\n" +
            "    },\n" +
            "    {\n" +
            "      \"id\": \"stage-1\",\n" +
            "      \"label\": \"발단 (2/2)\",\n" +
            "      \"content\": \"바로 앞 씬(1/2)과 동일한 장소·시간·인물을 유지하면서, 앞 씬에서 시작된 행동이나 감정이 자연스럽게 이어지거나 심화되는 300~500자 분량의 묘사.\"\n" +
            "     // elements 동일하게 포함 (background, time, mainCharacter는 1/2 씬과 일치시킬 것)\n" +
            "    }\n" +
            "    // ... 단계당 1/2, 2/2 형식으로 총 %d * 2 개의 요소 생성\n" +
            "  ]\n" +
            "}\n" +
            "content 작성 규칙: 각 씬을 영화 촬영 지문처럼 묘사해. ① 인물(표정·의상·감정 상태), ② 공간(장소·날씨·시간·빛), ③ 행동(카메라 움직임 포함), ④ 분위기(사운드·색감·감정 톤) 순으로 300~500자로 서술해. 단순 요약이 아닌 독자가 눈앞에 그릴 수 있을 정도로 생생하게 써.\n" +
